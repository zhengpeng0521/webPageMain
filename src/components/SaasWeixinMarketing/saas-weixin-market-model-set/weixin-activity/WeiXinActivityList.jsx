...
import style from './WeiXinActivity.less'


function WeiXinActivityList({
  ...
  const columns = [{
    ...
    render: (text, record) => (
        <div>
            ...
            {!!(record.type == 'activity' || record.type == 'leaflet') &&
        ...
        columns.push(
            {
                ...
                render:(text,record) =>(
                    <div>{'2' == text ?
                        (<Popconfirm title={<span>确定设置<strong style={{color:'red'}}>上架</strong>吗</span>} onConfirm={() => tableWeiXinActivityUp(record.id)}>
                            <a className={style.check}>下架中</a>
                        </Popconfirm>)
                        :'1' == text ?
                        (<Popconfirm title={<span>确定设置<strong style={{color:'#57c5f7'}}>下架</strong>吗</span>} onConfirm={() => tableWeiXinActivityDown(record.id)}>
	...
	let paginationProps = {
        current : weiXinActivityPageIndex + 1,
        pageSize : weiXinActivityPageSize,
        total: weiXinActivityTotal,
        showSizeChanger: true,
        showQuickJumper :true,
        onShowSizeChange : changeWeiXinActivityPageSize,
        onChange : changeWeiXinActivityPageIndex,
        showTotal(){
            ...
            <div className="common-over">
                ...
                <div className="common-right" style={{width : '40%'}}>
                    <div className="table-operations" >
                        ...
                        <Button type="primary" onClick={tableOnWeiXinActivityFilter}><Icon type="filter" />筛选</Button>
                    </div>
                </div>
            </div>
            <Table
                columns={columns}
                dataSource={weiXinActivityList}
                loading={weiXinActivityLoading}
                pagination={paginationProps}
                onChange={tablePageChangeWeiXinActivity}
                bordered
                rowKey="id"
            />
        </div>
...
export default WeiXinActivityList;
