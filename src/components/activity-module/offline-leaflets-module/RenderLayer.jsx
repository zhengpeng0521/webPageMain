...
function LayerComponent({
	...
	for(let i = itemConfig&&itemConfig.length - 1; i >= 0; i--) {
		elementArr.push(itemConfig[i])
	}
	...
	return (
		<div className="layerContent">
			...
			<div className={styles.layerListBox}>
				{
					elementArr&&elementArr.map((item, index) => {
						...
						return  <div key={index} className={styles.itemBox} >
									<div className={styles.visibilityBox} onClick={() => changeElementvisibility(item&&item.item.visibility, item.index)}>
										<div className={iconStyle}></div>
									</div>
									<div className={selectElementStyle} onClick={() => changeSelectElement(item.index)}>
