...
const HomePageModal = ({
  ...
  function handleComplete(e) {
    ...
    validateFieldsAndScroll((errors,fieldsValue) => {
      ...
      switch(data.status){
          case '上架' : data.status='1';break;
          case '下架' : data.status='0';break;
          case '内部测试' : data.status='9';break;
      }
    ...
    <Modal {...modalOpts}>
      <Form horizontal>
        ...
        <FormItem
          ...
          {...formItemLayout}
        >
          {getFieldDecorator('game_time', {
            rules: [
                { type: 'array', required: '', message: '请选择游戏时间' }
            ]
          })(
        ...
        >
          {getFieldDecorator('amountLimit', {
            initialValue: formData.amountLimit!=undefined?formData.amountLimit+'':'',
            rules: [
...
export default Form.create()(HomePageModal);
