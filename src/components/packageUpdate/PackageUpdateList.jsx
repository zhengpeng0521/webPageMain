...
function OrganMessageList({
  ...
  const rowSelection = {
    ...
    getCheckboxProps: record => ({
      disabled: record.isUpdate === "1" // Column configuration not to be checked
    })
  };
  const columns = [
    {
    ...
    },
    {
      ...
      dataIndex: "isUpdate",
      key: "isUpdate",
      render: (text, record) => <div>{text == "1" ? "已升级" : "未升级"}</div>
    },
    // {
    //   width: 100,
    //   title: "状态",
    ...
    //   key: "moid",
    //   render: (text, record) => (
    //     <a onClick={() => showDetilFun(record.tenantId, record.id, "zsb")}>
    //       {record.mealTitle}
    //     </a>
    //   )
    // },
    ...
    //   key: "saas",
    //   render: (text, record) => (
    //     <a onClick={() => showDetilFun(record.tenantId, record.id, "saas")}>
    //       {record.saasMeal}
    //     </a>
    //   )
    // },
    ...
    //           title={
    //             <p>
    //               确定设置<strong style={{ color: "red" }}>解冻</strong>吗?
    //             </p>
    //           }
    //           onConfirm={() =>
    //             tableOrganPackageNoFreeze(record, record.tenantId)
    //           }>
    ...
    //           title={
    //             <p>
    //               确定设置<strong style={{ color: "#77c8f8" }}>冻结</strong>吗?
    //             </p>
    //           }
    //           onConfirm={() =>
    //             tableOrganPackageFreeze(record, record.tenantId)
    //           }>
      ...
      <div className="common-over">
        ...
        <div className="common-right" style={{ width: "40%" }}>
          <div className="table-operations">
            <Button type="primary" onClick={packageUpdateFunc}>
              <Icon type="to-top" />
              套餐升级
            </Button>
            <Button type="primary" onClick={tableOnFilter}>
              <Icon type="filter" />
              筛选
            </Button>
          </div>
