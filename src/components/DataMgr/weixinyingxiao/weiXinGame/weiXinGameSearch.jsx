...
function WeiXinGameSearch ({
    ...
    let children = [];
    if(WeiXinGameSelectValue && WeiXinGameSelectValue.length>0){
        children = WeiXinGameSelectValue.map((item) => {
            return(
    ...
    function TimeOnSelect(dates, dateStrings){
        ...
        WeiXinGameTimeOnSelectSubmit(time);
    }
            ...
            <Form horizontal >
                <div style={{float:'left',margin:'20px'}}>
                    {getFieldDecorator('game', {
                        initialValue: WeiXinGameSelectValue&&WeiXinGameSelectValue.length>0?WeiXinGameSelectValue[0].title+'':'',
                    })(
                    <Select placeholder="请选择游戏" style={{ width: 150 }} onChange={WeiXinGameGameChangeSelect}>
                        {children || []}
                    </Select>
                )}
                </div>
