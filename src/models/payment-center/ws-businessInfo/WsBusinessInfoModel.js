...
import { queryWsMchInfoList , queryAppInfoList } from '../../../services/payment-center/ws-businessInfo/WsBusinessInfoService'
export default {

    namespace: 'WsBusinessInfoModel',
    ...
    subscriptions: {
        setup({ dispatch, history }) {
            history.listen(location => {
                if (location.pathname === '/ws_businessInfo') {
                    dispatch({
                        type : 'queryWsMchInfoList',
                        payload : {
                            pageIndex : 0,
                            pageSize : 10
                        }
    ...
    effects: {
        *queryWsMchInfoList({ payload }, { call, put, select }) {
            ...
            let { ret } = yield call(queryWsMchInfoList, parse(params));
