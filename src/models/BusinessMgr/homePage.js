import { create, remove, update, query, queryForlList,addEssence,clearCache,recommend,doUp,createFormSubmit,updateFormSubmit } from '../../services/BusinessMgr/homePage';
...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/main_page_mgr') {
            dispatch({
                type: 'queryForlList'
            });
        }
  ...
  effects: {
    ...
    *query({ payload }, { call, put, select }) {
        ...
        let homePage = yield select(state => state.homePage);
        let searchData = homePage.searchData || {};
        let pageIndex = homePage.pageIndex;
        let pageSize = homePage.pageSize;
    ...
    *'queryForlList'({ payload }, { call, put, select }) {
      yield put({ type: 'showLoading' });
      let secondSale = yield select(state => state.secondSale);
      let pageIndex = secondSale.pageIndex;
      let pageSize = secondSale.pageSize;
      let params = { pageIndex, pageSize };
      const { ret } = yield call(queryForlList, parse(params));
      if (ret && ret.errorCode === 9000) {
        yield put({
