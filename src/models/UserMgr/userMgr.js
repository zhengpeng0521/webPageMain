import { create, remove, update, query,
        ...
        batchAddEssence,batchRecommend,batchDoUp,batchClearCache,batchDelete,queryForSearchUser } from '../../services/UserMgr/userMgr';
...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/user_mgr') {
            dispatch({
                type: 'queryForSearchUser'
            });
        }
      });
    },
  ...
  effects: {
    ...
    *query({ payload }, { call, put, select }) {
        ...
        let userMgr = yield select(state => state.userMgr);
        let pageIndex = userMgr.pageIndex;
        let pageSize = userMgr.pageSize;
    ...
    *'queryForSearchUser'({ payload }, { call, put }) {
      yield put({ type: 'showLoading' });
      const { ret } = yield call(queryForSearchUser);
      if (ret && ret.errorCode === 9000) {
        yield put({
    ...
    *'export'({ payload } , { call , put , select }){
        window.open(`${BASE_URL}/user/userExport?${qs.stringify(payload)}`);
    },
