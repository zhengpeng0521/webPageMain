import { create, remove, update, query, queryForSearchInvite,addEssence,clearCache,recommend,doUp, } from '../../services/UserMgr/userInvite';
...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/user_invite') {
            dispatch({
                type: 'queryForSearchInvite'
            });
        }
      });
    },
  ...
  effects: {
    ...
    *query({ payload }, { call, put, select }) {
        ...
        let userInvite = yield select(state => state.userInvite);
        let searchData = userInvite.searchData || {};
        let pageIndex = userInvite.pageIndex;
        let pageSize = userInvite.pageSize;
    ...
    *'queryForSearchInvite'({ payload }, { call, put }) {
      yield put({ type: 'showLoading' });
      const { ret } = yield call(queryForSearchInvite);
      if (ret && ret.errorCode === 9000) {
        yield put({
          type: 'updateSearchInvite',
          payload: {
            list: ret.results,
            total: ret.data.resultCount,
            pageIndex : ret.data.pageIndex,
  ...
  reducers: {
    ...
    //更新查询框的频道列表
    updateSearchInvite(state, action) {
        return { ...state, ...action.payload };
    },
