import { create, remove, update, query, queryForSearchChannelList,addEssence,clearCache,recommend,doUp, } from '../../services/UserMgr/topicReward';
...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/user_topic_reward') {
            dispatch({
  ...
  effects: {
    ...
    *query({ payload }, { call, put, select }) {
        ...
        let topicReward = yield select(state => state.topicReward);
        let searchData = topicReward.searchData || {};
        let pageIndex = topicReward.pageIndex;
        let pageSize = topicReward.pageSize;
    ...
    *'export'({ payload } , { call , put , select }){
        window.open(`${BASE_URL}/user/topicRewardExport?${qs.stringify({...payload})}`);
    },
