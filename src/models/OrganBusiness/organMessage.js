import { queryForOrganMessage,checkIncludingModal,OrganMangeFreeze,freezeAfterOperation,getAllSaasPayment} from '../../services/OrganBusiness/organMessage';
  ...
  state: {
    ...
    subscriptions: {
        setup({ dispatch, history }) {
            history.listen(location => {
                if (location.pathname === '/organ_message') {
                    dispatch({
    ...
    effects: {
        ...
        *'AfterFreezeQuery'({ payload }, { call, put , select }){
            let organMessage = yield select(state => state.organMessage);
            let searchData = organMessage.searchData || {};
            let pageIndex = organMessage.pageIndex;
            let pageSize = organMessage.pageSize;
