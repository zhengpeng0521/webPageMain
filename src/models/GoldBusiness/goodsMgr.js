import { create, remove, update, query, queryForList,addEssence,clearCache,recommend,doUp,GoodsUpdate, GoodsCreate} from '../../services/GoldBusiness/goodsMgr';
...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/goods_mgr') {
            dispatch({
                type: 'queryForList'
            });
        }
  ...
  effects: {
    ...
    *query({ payload }, { call, put, select }) {
        ...
        let goodsMgr = yield select(state => state.goodsMgr);
        let searchData = goodsMgr.searchData || {};
        let pageIndex = goodsMgr.pageIndex;
        let pageSize = goodsMgr.pageSize;
    ...
    *'GoodsUpdate'({ payload }, { call, put, select }){
      yield put({ type: 'showLoading' });
      const { ret,err } = yield call(GoodsUpdate, parse(payload));
        if (ret && ret.errorCode === 9000) {
            yield put({
                type: 'query',
            });
    ...
    *'GoodsCreate'({ payload }, { call, put, select }){
      yield put({ type: 'showLoading' });
      const { ret,err } = yield call(GoodsCreate, parse(payload));
        if (ret && ret.errorCode === 9000) {
            yield put({
                type: 'query',
            });
