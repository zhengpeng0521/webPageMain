import { SearchModalOne } from '../../services/DataMgr/koubeishangpin';
...
export default {
  ...
  state: {
    ...
    dinggoumendianList:[],
    shangpinshuList:[],
    xinzengshangpinfugaimendianList:[],
    shangjiashangpinList:[],
    xinzengshangpinzongmendianshuList:[],
    chengjiaodingdanList:[],
    dingdanchengjiaojineList:[],
    hexiaoshuliangList:[],
    hexiaojineList:[],
    XTime:[],
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/koubeishangpin') {
            dispatch({
    ...
    *'SearchModalOne'({ payload }, { call, put, select }){
        ...
        if (ret && ret.errorCode === 9000) {
           yield put({
                type: 'updateState',
                payload: {
                        dinggoumendianList:ret.data.addShops,
                        shangpinshuList:ret.data.addProduct,
                        xinzengshangpinfugaimendianList:ret.data.addOrgGoods,
                        shangjiashangpinList:ret.data.addOrderGoods,
                        xinzengshangpinzongmendianshuList:ret.data.addAllGoods,
                        chengjiaodingdanList:ret.data.addDealOrder,
                        dingdanchengjiaojineList:ret.data.addPur,
                        hexiaoshuliangList:ret.data.addSettle,
                        hexiaojineList:ret.data.addSettlePay,
                        XTime : ret.data.time,
                },
            });
