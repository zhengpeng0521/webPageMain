import { SearchModalOne } from '../../services/DataMgr/shanshanguanjia';
...
export default {
  ...
  state: {
    ...
    shanghuList:[],  //商户数据
    mendianList:[],  //门店数据
    shanghuzhanghaoList:[],  //商户账号数据
    shanghudengluList:[],  //商户登录数据
    ...
    subscriptions: {
        setup({ dispatch, history }) {
            history.listen(location => {
                if (location.pathname === '/shanshanguanjia') {
    ...
    *'SearchModalOne'({ payload }, { call, put, select }){
        ...
        if (ret && ret.errorCode === 9000) {
           yield put({
                type: 'updateState',
                payload: {
                        shanghuList : ret.data.tenant,
                        mendianList : ret.data.store,
                        shanghuzhanghaoList : ret.data.acct,
                        shanghudengluList : [],
                        XTime : ret.data.time,
                },
            });
