import { AllList,SearchModalOne } from '../../services/DataMgr/shanshanzaojiao';
...
export default {
  ...
  state: {
    ...
    point : '',   //节点是否可点击 '1'可点击
    yonghuzhuceList:[], //用户注册列表
    yonghudengluList:[],  //用户登录列表
    qidongcishuList:[],  //启动次数列表
    jinbizengjiaList:[],  //金币增加列表
    jinbixiaohaoList:[],  //金币消耗列表
    tieziList:[],  //帖子列表
    XTime:[],
    ...
    subscriptions: {
        setup({ dispatch, history }) {
            history.listen(location => {
                if (location.pathname === '/shanshanzaojiao') {
    ...
    *'SearchModalOne'({ payload }, { call, put, select }){
        ...
        if (ret && ret.errorCode === 9000) {
           yield put({
                type: 'updateState',
                payload: {
                    yonghuzhuceList:ret.data.user, //用户注册列表
                    yonghudengluList:[],  //用户登录列表
                    qidongcishuList:[],  //启动次数列表
                    jinbizengjiaList:ret.data.goldAdd,  //金币增加列表
                    jinbixiaohaoList:ret.data.gold,  //金币消耗列表
                    tieziList:ret.data.topic,  //帖子列表
                    XTime:ret.data.time,
                },
            });
